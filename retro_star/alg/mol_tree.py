    ...
    def get_routes(self, optimal):
        ...
        while not mol_queue.empty():
            ...
            if optimal:
                ...
                #reactants_temp = Chem.MolToSmiles(Chem.MolFromSmiles(".".join(reactants).replace("[SAH]", "[32S]").replace("[CoA]", "[33S]")))
                #result = self.rerank_fn([reactants_temp.replace("[33S]", "[CoA]").replace("[32S]", "[SAH]")], mol.mol, [best_reaction.cost])
                syn_route.add_reaction(
                    mol=mol.mol,
                    value=mol.succ_value,
                    template=best_reaction.template,
            ...
            if len(all_reactions) > 1:
                for reaction in all_reactions:
                    ...
                    for reactant in reaction.children:
                        ...
                        reactants.append(reactant.mol)
                    #reactants_temp = Chem.MolToSmiles(Chem.MolFromSmiles(".".join(reactants).replace("[SAH]", "[32S]").replace("[CoA]", "[33S]")))
                    #result = self.rerank_fn([reactants_temp.replace("[33S]", "[CoA]").replace("[32S]", "[SAH]")], mol.mol, [reaction.cost])
                    syn_route.add_reaction(
                        mol=mol.mol,
